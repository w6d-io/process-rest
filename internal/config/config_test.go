    ...
    When("Parse yaml config", func() {
        ...
        Context("New", func() {
            ...
            It("failed because of validation", func() {
                dir, err := os.CreateTemp("", "post_dir")
                Expect(err).To(Succeed())
                _, err = os.CreateTemp(dir.Name(), "directory")
                Expect(err).To(Succeed())
                filename := dir.Name() + string(os.PathSeparator) + "script1.sh"
                err = os.WriteFile(filename, []byte(fileTest), 0644)
                Expect(err).To(Succeed())
                configFile := dir.Name() + string(os.PathSeparator) + "config.yaml"
                data := fmt.Sprintf(configTestFile, "post_script_folder", dir)
                err = os.WriteFile(configFile, []byte(data), 0444)
                Expect(err).To(Succeed())
            ...
            It("success", func() {
                dir, err := os.CreateTemp("", "process_dir")
                Expect(err).To(Succeed())
                _, err = os.CreateTemp(dir.Name(), "directory")
                Expect(err).To(Succeed())
                filename := dir.Name() + string(os.PathSeparator) + "script1.sh"
